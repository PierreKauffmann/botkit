...
const CSlogger = logger.Application;
...
function Facebookbot(configuration) {
	...
	facebook_botkit.defineBot(function (botkit, config) {
		...
		bot.getMessageUser = function (message, cb) {
			CSlogger.warn('in getMessageUser ' + JSON.stringify(message));
			try {
				...
				CSlogger.warn('in getMessageUser ' + error);
				return {};				
			}
	...
	facebook_botkit.handleWebhookPayload = function (req, res, bot) {
		...
		CSlogger.warn('handleWebhookPayload->', JSON.stringify(payload));
	...
	facebook_botkit.middleware.normalize.use(function normalizeMessage(bot, message, next) {
		CSlogger.info('->Comments','normalizeMessage', JSON.stringify(message))
		...
		if (message.field && message.field == 'sessions') {
			...
			CSlogger.info('->Comments','capture the user ID', JSON.stringify(message.sender))
			if (message.sender){
			// capture the user ID
	...
	var fAIbrikInfo = function (message) {
		CSlogger.warn('in fAIbrikInfo ' + JSON.stringify(message));
		try {
			facebook_botkit.api.user_profile(message.user).then(function (identity) {
			...
			CSlogger.warn('in fAIbrikInfo ' + error);
			return {};				
		}
	};

	var user_profile = function (uid, fields, cb) {
		CSlogger.warn('in user_profile ... ' + uid);
		if (!fields) {
		...
		try {
			...
			CSlogger.warn('user_profile...' + uri);
			var results = null;
			request.get(uri, {}, function (err, res, body) {
				if (err) {
					CSlogger.error('Could not get user profile:', err);
					return;
				} else {
					try {
						results = JSON.parse(body);
					} catch (err) {
						CSlogger.error('ERROR in user profile call: Could not parse JSON', err, body);
						return;
					}
				}
			});
			...
			CSlogger.error('In user_profile ... ' + error);
		}
	};
