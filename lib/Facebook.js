/**
 * This file
 * https://github.com/PierreKauffmann/botkit?organization=PierreKauffmann&organization=PierreKauffmann
 */
...
function Facebookbot(configuration) {
	...
	module.exports.FB_botkit = facebook_botkit;
	...
	facebook_botkit.defineBot(function (botkit, config) {
		...
		bot.getMessageUser = function (message, cb) {
			// facebook_botkit.log('in getMessageUser ' + JSON.stringify(message));
			try {
				facebook_botkit.api.user_profile(message.user, 'name,first_name,last_name,profile_pic')
					.then(function (identity) {
						...
						timezone_offset: identity.timezone || null,
						*/
						};
						return profile;
					});
			} catch (error) {
				facebook_botkit.log('in getMessageUser ' + error);
				return {};
			}
	...
	facebook_botkit.createWebhookEndpoints = function (webserver, bot, cb) {
		...
		/** fAIbrik specific */
		var faibrik_route = facebook_botkit.config.faibrik_route || 'facebook/receive';

		facebook_botkit.log(
			'** fAIbrik ... Serving webhook endpoints for Messenger Platform at: ' +
			'http://' + facebook_botkit.config.hostname + ':' + facebook_botkit.config.port + '/' + faibrik_route);
		webserver.post('/' + faibrik_route, function (req, res) {
			res.send('ok');
			facebook_botkit.handleWebhookPayload(req, res, bot);
		});

		webserver.get('/' + faibrik_route, function (req, res) {
	...
	facebook_botkit.handleWebhookPayload = function (req, res, bot) {
		...
		// facebook_botkit.log('handleWebhookPayload->', JSON.stringify(payload));
	...
	facebook_botkit.middleware.normalize.use(function normalizeMessage(bot, message, next) {
		// facebook_botkit.log('->Comments', 'normalizeMessage', JSON.stringify(message))
		...
		if (message.field && message.field == 'sessions') {
			...
			if (!message.sender) {
				if (message.optin && message.optin.user_ref) {
					message.sender = {
						id: message.optin.user_ref
					};
				}
				if (message.field == 'feed') {
					// facebook_botkit.log('->Comments', 'WE HAVE A FEED make up sender')
					message.sender = message.value.from
				}
			}
			// capture the user ID
			...
			if (message.field != 'feed') {
				// facebook_botkit.log('->Comments', 'WE HAVE A FEED make up page')
				message.page = message.recipient.id;
			} else {
				message.page = message.value.post_id
			}
		}
	...
	var fAIbrikInfo = function (message) {
		// facebook_botkit.log('in fAIbrikInfo ' + JSON.stringify(message));
		try {
			facebook_botkit.api.user_profile(message.user).then(function (identity) {
			...
			facebook_botkit.log('in fAIbrikInfo ' + error);
			return {};
		}
	};

	var user_profile = function (uid, fields, cb) {
		// facebook_botkit.log('in user_profile ... ' + uid);
		if (!fields) {
			fields = 'name,first_name,last_name,profile_pic'; //name, first_name, last_name, and profile_pic
		...
		try {
			...
			//         https://graph.facebook.com/1462570350526759?fields=first_name,last_name,profile_pic&access_token=EAAcg8GtkG3ABABgdgw0dV9amRZBAiOGZAnuSfif2zmwZCBHfylrxzSfxM10ayd5ZCXNUoBHFs8G8uRPluHANWSz1K1FDkeY4LcOCcvRwVjF63PVCuUSdEKz0u2Tnmoi3FOY42YdMZB9TWT2IxO5ZAGR00uCB4Dv6yhYriMmx5lAwZDZD
			if (facebook_botkit.config.require_appsecret_proof) {
				uri += '&appsecret_proof=' + appsecret_proof;
			}
			// facebook_botkit.log('user_profile...' + uri);
			var results = null;
			...
			facebook_botkit.log('In user_profile ... ' + error);
		}
	};
